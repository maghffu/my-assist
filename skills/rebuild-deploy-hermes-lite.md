...
- **MIGRATION RACE — INSIDEN 5 Sep 00:00 (downtime 2 menit): binary lama + `_sqlx_migrations` baru = crash loop saat restart.** sqlx menolak start kalau DB punya baris migration versi lebih baru dari yang di-embed binary (`migration N was previously applied but is missing in the resolved migrations`). "Additive-only" melindungi SCHEMA, BUKAN startup validation. Konsekuensi:
  1. **JANGAN apply migration manual ke DB prod sebelum binary yang memuatnya ter-deploy** — termasuk `cargo run -- migrate` dari repo checkout di VPS (`.env` repo menunjuk DB yang SAMA dengan service: `localhost:5432/hermes` — bukan dev DB terpisah!). Migration harus masuk lewat deploy normal (auto-run saat startup binary baru).
  2. Kalau terlanjur: DB bisa dibiarkan (binary baru akan datang ≤5 menit via poll-deploy) — crash loop hanya kena kalau ada restart di window itu; TIDAK perlu DROP (dropping malah bikin binary baru nanti re-apply — aman sih, tapi sia-sia).
  3. Recovery tercepat kalau sudah crash-loop: tunggu poll-deploy pasang build baru (5 menit), ATAU `/rollback` TIDAK menolong (binary rollback lebih lama lagi) — yang menolong hanya build yang memuat migration tsb.
