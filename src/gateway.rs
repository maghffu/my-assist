...
async fn handle_message(bot: Bot, msg: Message, agent: Arc<Agent>) -> Result<()> {
    ...
    // Dokumen (file attachment) → download → simpan → turn agent (pasangan Pilar 7):
    ...
    let text = match msg.text() {
        ...
        _ => {
            bot.send_message(
                msg.chat.id,
                "🤔 Aku belum bisa memproses tipe pesan ini (voice/sticker/lokasi/dll.).\n\
                 Kirim teks langsung, screenshot sebagai foto, atau file sebagai dokumen.",
            )
            .await?;
            return Ok(());
        }
...
/// Batas ukuran dokumen (Telegram Bot API getFile maks 20MB — dijaga sama).
